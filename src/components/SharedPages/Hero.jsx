  ...
  return (
    <section className="hero">
      <div className="w-full h-auto md:h-[92vh] md:flex md:justify-between">
        ...
        {/* Right Grid */}
        {/* 
          We set h-[92vh] to match the slider height
          and use grid-cols-2 + grid-rows-2 to make a 2x2 grid 
          for the four hero2 images.
        */}
        <div className="w-full md:w-1/2 grid grid-cols-2 grid-rows-2 h-[92vh] sm:grid-cols-1 sm:grid-rows-4">
          {hero2.map((item) => (
            <div key={item.id} className="box relative w-full h-full">
              <img
                src={item.cover}
                alt="cover"
                className="w-full h-full object-cover"
